...
```
\- name, version<br />
package.json 에서 가장 중요한 항목은 "name"과 "version"이며, 필수로 입력되어야 하며 이 항목들이 누락되면 패키지는 설치할 수 없습니다.<br />
\- author<br />
...
```
\- description<br />
설명을 문자열로 기술하고 npm search로 검색가능 검색 리스트에 노출시켜줍니다.
\- keywords<br />
키워드를 문자열 배열로 설명하고 npm search로 검색가능 검색 리스트에 노출시켜줍니다.
\- dependencies<br />
패키지의 배포 시 포함될 의존성 모듈을 지정합니다.
\- devDependencies<br />
패키지의 개발 시 사용될 의존성 모듈을 지정합니다. (배포 시 포함되지 않습니다)
\- engines<br />
동작 가능한 node의 버젼을 지정할 수 있습니다.
\- repository<br />
코드가 존재하는 장소를 지정합니다.
